 ...
 * @author Ico
 */
class sfWidgetFormViettelEditable extends sfWidgetForm {
...
//      if(count($dataQueryList)>0){}
      ...
      foreach ($dataQueryList as $record) {
        ...
        $showWidget = new sfWidgetFormChoice(
          array('choices' => array(
            1 => "Chờ nghe máy"
          ,2 => "Trong khi nghe máy"
          ,3 => "Sau khi nghe máy"
          ),
            'multiple' => false,
            'expanded' => false
        ...
        $edit = new sfWidgetFormInputText(array('default'=>0), array('maxlength' => 15));
        ...
        foreach ($visibleColumns as $column) {
          ...
          if($column == "showType"){ //for campaign
            $tr = $tr . '<td>'.$showWidget->render('vt_campaign_avatar[show_type_'.$record['id'].']',$record[$column],array('style'=>'width:100px')).'</td>';
//            $tr = $tr . '<td><span title="'.VtCampaignAvatarTable::getShowType($record[$column]).'">' . VtCampaignAvatarTable::getShowType($record[$column]) . '</span></td>';
          }elseif($column == "budgetPoint"){
            $tr = $tr . '<td>'.$edit->render('vt_campaign_avatar[budget_point_'.$record['id'].']',$record[$column],array('style'=>'width:100px','class'=> 'validate[positiveInteger]')).'</td>';
          } elseif($column == "budget"){
            $tr = $tr . '<td>'.$edit->render('vt_campaign_avatar[budget_'.$record['id'].']',$record[$column],array('style'=>'width:100px','class'=> 'validate[positiveInteger]')).'</td>';
          }
          else {
