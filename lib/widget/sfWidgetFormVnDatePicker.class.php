...
class sfWidgetFormVnDatePicker extends sfWidgetFormInputText
{
  /**
   * Constructor.
   *
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    $epoch_date = strtotime(str_replace('/', '-', $value));
    if ($value)
      $value = date('d-m-Y', $epoch_date);
    ...
    if (!empty($epoch_date)) {
      $html .= $widget_month->render($name . '[month]', date('m', $epoch_date));
      $html .= $widget_day->render($name . '[day]', date('d', $epoch_date));
