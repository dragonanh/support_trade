 ...
 */
class sfValidatorDateTimePicker extends sfValidatorBase
{
  /**
  ...
  protected function configure($options = array(), $messages = array())
  {
    ...
    $this->addMessage('bad_format', $i18n->__('"%value%" does not match the date format (%date_format%).'));
    $this->addMessage('max', $i18n->__('The date must be before %max%.'));
    $this->addMessage('min', $i18n->__('The date must be after %min%.'));

    $this->addOption('date_format', '~(?P<day>\d{2})/(?P<month>\d{2})/(?P<year>\d{4}) (?P<hour>\d{2}):(?P<minute>\d{2}):(?P<second>\d{2})~');
    $this->addOption('with_time', true);
    $this->addOption('date_output', 'd/m/Y H:i:s');
  ...
  protected function doClean($value)
  {
    ...
    return $clean;//isset($date) ? $date->format($format) : date($format, $cleanTime);
  }
