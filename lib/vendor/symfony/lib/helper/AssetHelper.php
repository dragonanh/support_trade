...
function include_seo_metas(){
    ...
    echo tag('meta', array('property' => 'og:url', 'content' => null === $i18n ? $content : $i18n->__($content)))."\n";
    echo tag('meta', array('property' => 'og:image', 'content' => null === $i18n ? $content : $i18n->__($content)))."\n";
}
...
/*
function include_propertyMetas()
{
    $context = sfContext::getInstance();
    $i18n = sfConfig::get('sf_i18n') ? $context->getI18N() : null;
    foreach ($context->getResponse()->getMetas() as $property => $content)
    {
        echo tag('meta', array('property' => $property, 'content' => null === $i18n ? $content : $i18n->__($content)))."\n";
    }
}
/**
