 ...
 */
class sfDoctrineGroupByPager extends sfPager implements Serializable
{
  protected
    $connection        = null,
    $query             = null,
  ...
  public function init()
  {
    ...
    $param = $countQuery->getParams();
    $param = $param['where'];
    $countQuery = $countQuery->getSqlQuery();
    $countQuery = 'SELECT COUNT(*) as count FROM (' . $countQuery . ') num';
    $q = !$this->connection ? Doctrine_Manager::getInstance()->getCurrentConnection() : Doctrine_Manager::getInstance()->getConnection($this->connection);
    $result = $q->execute($countQuery, $param);
